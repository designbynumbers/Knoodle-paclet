...
| `"Orientation"` | `False` | `True` marks the knot's orientation with one arrowhead per link component (placed on that component's longest arc — the knot-theory convention: orientation propagates through crossings, so one arrow per component determines it everywhere). `All` puts an arrowhead on every arc. Design (2026-07-03, grounded in ASME Y14.2-style drafting practice — solid-filled heads, sized proportionally to line weight, one consistent style): the head is a solid swept-back kite (`$arrowShape`, unit length × 0.72 wide, tail notch at 0.75) with no color directive of its own so it inherits each strand's color; length is `$arrowLengthFactor` (3.5) × the *resolved* stroke width, so heads scale down with the adaptive `"Thickness"` on dense diagrams and cap with the 7 pt stroke cap on small ones. Placement: at the arc-length midpoint of the arc's **longest single segment** (computed on the same rounded polyline that is drawn) — always the middle of a straight run, never on a rounded corner (direction would be ambiguous) and never at an arc endpoint, so a head can *not* sit in or visually close the under-strand break at a crossing. When `"Labels"` includes `"Arcs"`, the anchor slides from 50% to 30% of that segment, because arc labels anchor at the midpoint of the longest horizontal segment — typically the same spot. Arc polylines are emitted by `knoodledraw` in orientation order (verified: per-arc arrows circulate consistently), so `Arrow` over the same points is correct as-is. Unknot-marker summands (`<\|"Unknot"->True\|>`) carry no arc data and draw without an arrowhead. |
